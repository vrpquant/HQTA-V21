# =============================================================================
# VRP QUANT ENGINE — v30.5.0-A+
# 5-Source Waterfall (Polygon + Alpha Vantage + Google Finance + Grok + yfinance)
# 200-step CRR Binomial + 10k-100k MC + Merton Jump-Diffusion
# =============================================================================
#
# DATA WATERFALL (OHLCV History):
#   1. Polygon.io free tier -> unlimited EOD aggregates
...
#   ✅ 200-step CRR Binomial Tree (vectorised backward induction)
...
logger = logging.getLogger("VRP_Quant_v30_5_0")


# =============================================================================
# [A+01] THREAD-SAFE TTL CACHE — replaces unbounded LOCAL_CACHE dict
# =============================================================================
...
# [A+01] Module-level cache instance — thread-safe, TTL-bounded
...
class QuantMath:
    ...
    CRR binomial tree (200 steps default), Monte Carlo simulation
    (10k–100k paths) with optional Merton jump-diffusion, Newton-Raphson
    IV solver, true POP with premium, and full P&L analytics.
    ...
    def crr_binomial_tree(
        ...
        """
        CRR (Cox-Ross-Rubinstein) binomial tree pricer.

        Vectorised backward induction. European pricing by default.
        Also returns the empirical POP from terminal node distribution.

        Args:
            n_steps: Number of tree steps (200 default — converges to <0.1% of BS).

        Returns:
            dict with keys: price, pop, n_steps.
        """
        if T <= 1e-10:
            intrinsic = max(0, S - K) if option_type == "call" else max(0, K - S)
            return {"price": round(intrinsic, 4), "pop": 0.0, "n_steps": n_steps}
        ...
        # POP for SHORT seller: short put profits when S_T > K, short call when S_T < K
        pop = float(np.mean(ST > K)) if option_type == "put" else float(np.mean(ST < K))
        return {"price": round(price, 4), "pop": round(pop, 4), "n_steps": n_steps}
...
class PolygonClient:
    """
    Polygon.io REST client — free tier (EOD + 15-min delayed snapshots).

    Capabilities (free tier):
      ✅ Daily OHLCV aggregates (unlimited history)
      ✅ 15-min delayed snapshots (quotes)
      ✅ Ticker reference + news
      ❌ Real-time websocket (requires Starter $29/mo)
      ❌ Options chain snapshots (requires Options add-on)
    """
...
class GrokFinanceClient:
    ...
    Uses the OpenAI-compatible endpoint at api.x.ai/v1 with grok-4-1-fast
    (cheapest model: $0.20/M input tokens — $175/mo free credits available).

    Requires GROK_API_KEY. Sign up free at console.x.ai.
    Enable data sharing for $150/mo free credits on top of $25 signup bonus.
    ...
    GROK_MODEL    = "grok-4-1-fast-non-reasoning"  # Verified April 2026: $0.20/M in, $0.50/M out, 2M context
    ...
    def get_live_quote(self, ticker: str) -> dict:
        """Fetch current price via Grok AI search (has real-time X/web access)."""
        if not self.available:
        ...
        try:
            ...
            if price and float(price) > 0:
                return {
                    "price": float(price),
                    "source": f"grok:{self.GROK_MODEL}",
                    "fetched_at": datetime.now(timezone.utc),
                }
            return {"price": None, "source": "grok:no_price"}
        except Exception as e:
            logger.warning(f"[{ticker}] Grok quote failed: {e}")
            return {"price": None, "source": "grok:failed"}
...
def fetch_history(ticker: str, period: str = "2y") -> pd.DataFrame:
    ...
    # Source 1: Polygon (free tier, unlimited EOD)
...
# =============================================================================
# [A+03] EXPECTED MOVE ENGINE — correct log-normal bounds
# =============================================================================
...
class OptionsExpectedMove:
    ...
    [A+03] Uses correct log-normal bounds S×exp(±σ√T) instead of the
    symmetric approximation S ± Sσ√T. The log-normal distribution is
    right-skewed, so upper distance > lower distance from spot.
    """
    ...
    def from_iv(S: float, T: float, sigma: float) -> dict[str, float]:
        ...
        # [A+03] Correct log-normal bounds (asymmetric)
...
# [A+02] Module load confirmation via logger, not print()
logger.info("VRP Quant Engine v30.5.0-A+ loaded — 5-Source Waterfall + 200-step CRR + 50k MC + Jump-Diffusion + WFO")
