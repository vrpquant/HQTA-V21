...
class BacktestEngine:
    ...
    def run_quick_backtest(df, slippage_bps=5, commission_bps=2):
        ...
        if len(wins) and len(losses):
            half_kelly = max(0, min(100, ((wins.mean() / losses.mean().abs()) - (1 - len(wins)/len(bt_df))) * 50))
    ...
    def walk_forward_validation(df, window=252, step=63):
        ...
        for i in range(window, len(df), step):
            ...
            _, cum_ret, _, _, _ = BacktestEngine.run_quick_backtest(train)
            test_ret = (test['Close'].pct_change() * np.where(train['SMA50'].iloc[-1] > train['SMA200'].iloc[-1], 1, -1)).sum()
            returns.append(test_ret)
        return round(np.mean(returns)*100,1) if returns else 0.0
...
class QuantLogic:
    ...
    @staticmethod
    def cross_sectional_alpha(score, peers_scores):
        return round((score - np.mean(peers_scores)) / np.std(peers_scores) * 100,1) if len(peers_scores)>1 else 0
...
class MonteCarloEngine:
    ...
    @staticmethod
    def stochastic_vol_fat_tail_strategy_mc(df, days=30, sims=1000, half_kelly=50):
        price = df['Close'].iloc[-1]
        ...
        paths = np.zeros((days+1, sims))
        paths[0] = price
        positions = np.where(np.random.rand(sims) > 0.5, 1, -1) * (half_kelly/100)
        for t in range(1, days+1):
            dW = np.random.normal(0,1,sims)
            dv = 0.5 * (0.04 - v) * dt + 0.3 * np.sqrt(v) * np.sqrt(dt) * dW
            v = np.maximum(v + dv, 1e-6)
            ret = mu*dt + np.sqrt(v*dt)*dW + np.random.standard_t(4,sims)*0.01
            paths[t] = paths[t-1] * (1 + ret * positions)
        strat_ret = (paths[-1]/price - 1).mean() * 100
        return round(strat_ret,1)
        ...
        for t in tickers:
            try:
                ...
                if len(df) > 50:
                    ...
                    vrp = QuantLogic.calculate_vrp_edge(t, df)
...
if check_login():
    ...
    if mode == "🚀 Market Scanner":
        ...
        st.title("🔬 Deep Dive & Trade Architect V22.2")
        ...
        if st.button("Run Analysis"):
            with st.spinner("Extracting from Institutional Vaults..."):
                try:
                    if os.path.exists("historical_vault.csv") and os.path.exists("latest_scan.csv"):
                        ...
                        if not df.empty and not ticker_stats.empty:
                            # Titanium Date Fix
                            ...
                            vol = float(str(stats['Vol']).replace('%', '').strip())
                            sup = float(stats['Support'])
                            res = float(stats['Resistance'])
                            vrp_edge = str(stats['VRP Edge'])
                            win_rate = float(str(stats['Win Rate']).replace('%', '').strip())
                            max_dd = float(str(stats['Max DD']).replace('%', '').strip())
                            half_kelly = float(str(stats['Kelly']).replace('%', '').strip())
                            ...
                            st.error(f"⚠️ {ticker} not found in the Data Vault. Did you scan it this morning?")
                    else:
                        st.error("⚠️ Pipeline link severed: CSV files not found. Upload them to GitHub.")
                except Exception as e:
