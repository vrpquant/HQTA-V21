...
# =============================================================================
# VRP QUANT LIVE TERMINAL — v30.5.0-A+ (COMPLETE PRODUCTION)
# 5-Source Waterfall + 200-step CRR + 10k-100k MC + Jump-Diffusion + bcrypt
# =============================================================================
...
from vrp_quant_engine_v30_5_0 import (
    QuantMath,
    PolygonClient,
    AlphaVantageClient,
    GoogleFinanceClient,
...
if not st.session_state["authenticated"]:
    st.title("🔒 VRP Quant Terminal v30.5.0")
...
with st.sidebar:
    ...
    if st.button("🔄 Clear Cache"):
        from vrp_quant_engine_v30_5_0 import LOCAL_CACHE
        LOCAL_CACHE.clear()
        st.cache_data.clear()
    ...
    if st.button("▶ Run Deep Dive", use_container_width=True):
        with st.spinner("Running GARCH + 200-step CRR + 50k MC + Jump-Diffusion..."):
            try:
                ...
                # CRR Binomial
                st.markdown("### 🌳 Pricing Convergence")
                binom = QuantMath.crr_binomial_tree(curr, sup, T30, rfr, sigma, n_steps=200, option_type="put")
                st.caption(f"CRR 200-step: ${binom['price']:.4f}  |  BS: ${bs['price']:.4f}  |  "
                           f"Error: {abs(binom['price']-bs['price'])/max(bs['price'],0.001)*100:.3f}%")
